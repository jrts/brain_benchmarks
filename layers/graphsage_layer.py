...
class GraphSageLayer(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, activation, dropout,
        ...
        if self.batch_norm:
            ...
            self.batchnorm_h = Norm('gn', out_feats)
            self.batchnorm_e = Norm('bn', out_feats)
    ...
    def forward(self, g, h, e):
        ...
        if self.dgl_builtin == False:
            ...
            if self.aggregator_type == 'maxpool':
                ...
                g.update_all(self.message_func, fn.max('m', 'c'), self.nodeapply)
            elif self.aggregator_type == 'lstm':
                g.update_all(self.message_func, self.aggregator, self.nodeapply)
            else:
