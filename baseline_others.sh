...
for model in PSCR
do
  for dataset in taowu_aal116_40 neurocon_aal116_41 matai_aal116_60 taowu_schaefer100_40 neurocon_schaefer100_41 matai_schaefer100_60
  do
    for L in 2 3
    do
      for hidden_dim in 'none' 64 100
      do
          python main.py --model=$model --config_dir='configs_01' --dataset=$dataset --L=$L --hidden_dim=$hidden_dim --epochs=80 \
                         --gpu_id=1 --batch_size=4 --save_base_url='../results_benchmarks'
      done
    done
  done
done