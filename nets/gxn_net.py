...
from layers.gxn_layer import GraphConvWithDropout, IndexSelect, GraphPool, GraphUnpool
...
class GraphClassifier(torch.nn.Module):
    ...
    def __init__(self, net_params):
        ...
        self.gxn = GraphCrossNet(in_dim=net_params['in_dim'],
                                 out_dim=net_params['out_dim'],
                                 # edge_feat_dim=net_params['edge_dim'],
        ...
        self.lin1 = torch.nn.Linear(net_params['out_dim'], net_params['out_dim'])
        self.lin2 = torch.nn.Linear(net_params['out_dim'], net_params['n_classes'])
        self.dropout = net_params['dropout']
        self.e_feat = net_params['edge_feat']

    def forward(self, graph, node_feat, edge_feat=None):
        # embed, logits1, logits2 = self.gxn(graph, node_feat, edge_feat)
        hg, logits1, logits2 = self.gxn(graph, node_feat, edge_feat=None)

        logits = F.relu(self.lin1(hg))
        if self.dropout > 0:
