...
from .components.transformer_encoder import InterpretableTransformerEncoder
...
class BrainNetworkTransformer(nn.Module):
    ...
    def __init__(self, net_params):
        ...
        forward_dim = net_params['hidden_dim']

        self.pos_encoding = net_params['pos_encoding']
        if self.pos_encoding == 'identity':
            self.node_identity = nn.Parameter(torch.zeros(
                net_params['in_dim'], net_params['pos_embed_dim']), requires_grad=True)
            forward_dim = net_params['in_dim'] + net_params['pos_embed_dim']
            nn.init.kaiming_normal_(self.node_identity)

        sizes = net_params['sizes']
        sizes[0] = net_params['in_dim']
        in_sizes = [net_params['in_dim']] + sizes[:-1]
        do_pooling = net_params['pooling']
        ...
        for index, size in enumerate(sizes):
            self.attention_list.append(
                TransPoolingEncoder(input_feature_size=forward_dim,
                                    ...
                                    orthogonal=net_params['orthogonal'],
                                    freeze_center=net_params['freeze_center'],
                                    project_assignment=net_params['project_assignment']))
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(256, 32),
            nn.LeakyReLU(),
            nn.Linear(32, net_params['n_classes'])
        )

    def forward(self, data):

        node_feature = data.x.view(-1, data.x.shape[-1], data.x.shape[-1])
