...
class PRGNNNet(torch.nn.Module):
    def __init__(self, net_params, poolmethod='sag'):
        super(PRGNNNet, self).__init__()
        ...
        self.conv1 = GATConv(indim, in_dim, num_heads, dropout, dropout)
        if self.poolmethod == 'topk':
            self.pool1 = TopKPooling
        elif self.poolmethod == 'sag':
            self.pool1 = SAGPool(hidden_dim*num_heads, ratio=self.ratio)  # 0.4 data1 10 fold

        self.conv2 = GATConv(in_dim*num_heads, in_dim, num_heads, dropout, dropout)
        if self.poolmethod == 'topk':
            self.pool2 = TopKPooling
        elif self.poolmethod == 'sag':
            self.pool2 = SAGPool(hidden_dim*num_heads, ratio=self.ratio)

        self.fc1 = torch.nn.Linear(hidden_dim*num_heads*4, out_dim)
